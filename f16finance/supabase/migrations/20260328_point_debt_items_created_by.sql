-- Track who physically added the debt (scanner operator) vs who owes it (debtor operator).
-- Allows the creator to delete a debt they added by mistake, even if it's assigned to someone else.
...
create index if not exists idx_point_debt_items_created_by
  on public.point_debt_items(created_by_operator_id);
