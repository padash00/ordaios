alter table public.point_returns
  add column if not exists sale_id uuid null references public.point_sales(id) on delete set null;

create index if not exists point_returns_sale_idx
  on public.point_returns (sale_id, returned_at desc)
  where sale_id is not null;

alter table public.point_return_items
  add column if not exists sale_item_id uuid null references public.point_sale_items(id) on delete set null;

create index if not exists point_return_items_sale_item_idx
  on public.point_return_items (sale_item_id)
...
begin
  ...
  loop
    ...
    select
      coalesce(sum(psi.quantity), 0),
      min(psi.id)
    into v_sold_qty, v_sale_item_id
